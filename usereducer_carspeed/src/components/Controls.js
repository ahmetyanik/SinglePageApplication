...
function Controls({state,dispatch}) {
    ...
    function isStarted(){
        ...
        if(state.engine===false){
            ...
            dispatch({type:"start", payload:{engine:state.engine, currentGear : state.currentGear}})
    ...
    return (
        <div>
            ...
            <button onClick={gearUp} disabled={!state.engine || state.currentGear===5}>Gear up</button>
            <button onClick={gearDown} disabled={!state.engine || state.currentGear===-2}>Gear down</button>
        </div>
    )
}
